...
extraction = pd.read_csv('./data/extractionDDI_34268.csv')
...
def BERT_train(training_generator, Testing_generator, embeddingtable1, embeddingtable2):
    ...
    with open('./2Baselines/MMDDI2-loss.csv', "a", newline='', encoding='utf-8') as csvfile:
...
def RecommendTest(data_generator, recommender):
    ...
    with open('./2Baselines/MMDDI2-each' + '.csv', "a", newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(headers)
        with torch.no_grad():
                ...
                for drugA_name in test_data['drugA'].unique():
                    ...
                    for rank, drug in enumerate(recommend_drugs_id):
                        ...
                        if drug == pos_candidate[0]:
                            ...
                            mrr += 1 / (rank + 1)  # MRR
                            break  # 只计算第一个命中
                writer.writerow([hits / len(test_data['drugA'].unique()), ndcg / len(test_data['drugA'].unique()),
                                 mrr / len(test_data['drugA'].unique())])
                if (batch_idx % 20 == 0):
...
def case_analysis(anchor_drug_list, model):
    ...
    for anchor_drug in anchor_drug_list:
        ...
        results = pd.DataFrame({
            ...
            'Attention_head1': attn[:, 0].cpu().numpy(),
            'Attention_head2': attn[:, 1].cpu().numpy(),
        })
...
def roc_aupr_score(y_true, y_score, average="macro"):
    def _binary_roc_aupr_score(y_true, y_score):
        precision, recall, pr_thresholds = precision_recall_curve(y_true, y_score) # 可以探索一下 pr_thresholds是什么，print一下
    ...
    def _average_binary_score(binary_metric, y_true, y_score, average):  # y_true= y_one_hot
        ...
        for c in range(n_classes):
            y_true_c = y_true.take([c], axis=1).ravel() # y_true 数组中取出索引为 c 的列，这列包含了所有样本是否属于类别 c 的标签（通常是一个二进制标签）
...
def cross_val(embeddingtable1, embeddingtable2):
    ...
    headers = [f"HR@{topk}", f"NDCG@{topk}", f"MRR@{topk}", f"Recall@{topk}"]
    with open('./2Baselines/MMDDI2-all' + '.csv', "a", newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(headers)
